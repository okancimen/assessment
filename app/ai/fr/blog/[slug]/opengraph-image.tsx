...
import { getFrenchPostBySlug } from '@/app/blog/posts-fr'
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = post?.shortTitle ?? 'Recherche sur les stages et la carrière'
  const tag = post?.tags[0] ?? 'Stage'

...
import { getTurkishPostBySlug } from '@/app/blog/posts-tr'
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = post?.shortTitle ?? 'Staj ve Kariyer Araştırması'
  const tag = post?.tags[0] ?? 'Staj'

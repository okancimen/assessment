...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: { ru: url, en: enUrl, 'x-default': enUrl },
    },
    ...
    openGraph: {
      ...
      images: [{ url: `/ru/blog/${slug}/opengraph-image`, width: 1200, height: 630, alt: post.title }],
    },
    twitter: {
      card: 'summary_large_image',
      title: post.title,
      description: post.description,
      images: [`/ru/blog/${slug}/opengraph-image`],
    },
  }
}
...
export default async function RUBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = BLOG_POSTS_RU.filter((p) => p.slug !== slug).slice(0, 3)
  ...
  return (
    <main lang="ru" className="max-w-3xl mx-auto px-6 py-16">
      ...
      <nav className="text-sm text-gray-400 mb-8">
        <Link href="/ru" className="hover:text-gray-600">Главная</Link>
        <span className="mx-2">›</span>
        <Link href="/ru/blog" className="hover:text-gray-600">Блог</Link>
        <span className="mx-2">›</span>
      ...
      <div className="mb-12">
        ...
        <div className="flex items-center gap-3 text-sm text-gray-400">
          ...
          <span>·</span>
          <span>Исследовательская команда Eduentry</span>
        </div>
      </div>
      ...
      {related.length > 0 && (
        <section className="mt-16 mb-12">
          <h2 className="text-xl font-bold text-gray-900 mb-6">Похожие статьи</h2>
      ...
      <section className="mt-16 mb-8 border border-gray-100 rounded-2xl p-8">
        <h2 className="text-xl font-bold text-gray-900 mb-2">Готов начать? Выбери своё направление.</h2>
        <p className="text-sm text-gray-400 mb-6">Бесплатная оценка из 34 вопросов — 35 минут. Выбери направление, в котором хочешь развиваться.</p>
        <div className="grid sm:grid-cols-2 gap-3">
          {[
            { href: '/tech',              icon: '💻', label: 'Технологии',        desc: 'Программирование · Алгоритмы · Кибербезопасность' },
            { href: '/business',          icon: '📈', label: 'Бизнес',            desc: 'Стратегия · Финансы · Операции' },
            { href: '/data-analytics',    icon: '📊', label: 'Анализ данных',     desc: 'Графики · SQL · Аналитика' },
            { href: '/digital-marketing', icon: '📣', label: 'Цифровой маркетинг', desc: 'SEO · Соцсети · Кампании' },
      ...
      <section className="text-center bg-indigo-600 rounded-2xl p-12 text-white">
        <h2 className="text-3xl font-bold mb-4">{post.cta?.heading ?? 'Узнай свой уровень готовности'}</h2>
        <p className="text-indigo-200 mb-8">{post.cta?.body ?? 'Бесплатная адаптивная оценка для старшеклассников старше 14 лет.'}</p>
        <CtaLink href={post.cta?.href ?? '/apply'} label="ru_blog_cta" className="bg-white text-indigo-600 px-8 py-4 rounded-xl font-semibold hover:bg-indigo-50 transition-colors text-lg">

...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: { zh: url, en: enUrl, 'x-default': enUrl },
    },
    ...
    openGraph: {
      ...
      images: [{ url: `/zh/blog/${slug}/opengraph-image`, width: 1200, height: 630, alt: post.title }],
    },
    twitter: {
      card: 'summary_large_image',
      title: post.title,
      description: post.description,
      images: [`/zh/blog/${slug}/opengraph-image`],
    },
  }
}
...
export default async function ZHBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = BLOG_POSTS_ZH.filter((p) => p.slug !== slug).slice(0, 3)
  ...
  return (
    <main lang="zh" className="max-w-3xl mx-auto px-6 py-16">
      ...
      <nav className="text-sm text-gray-400 mb-8">
        <Link href="/zh" className="hover:text-gray-600">首页</Link>
        <span className="mx-2">›</span>
        <Link href="/zh/blog" className="hover:text-gray-600">博客</Link>
        <span className="mx-2">›</span>
      ...
      <section className="mt-16 mb-8 border border-gray-100 rounded-2xl p-8">
        <h2 className="text-xl font-bold text-gray-900 mb-2">准备好开始了吗？选择你的方向。</h2>
        <p className="text-sm text-gray-400 mb-6">免费34道题评估 — 35分钟。选择你想发展的方向。</p>
        <div className="grid sm:grid-cols-2 gap-3">
          {[
            { href: '/tech',              icon: '💻', label: '科技',     desc: '编程 · 算法 · 网络安全' },
            { href: '/business',          icon: '📈', label: '商业',     desc: '战略 · 财务 · 运营' },
            { href: '/data-analytics',    icon: '📊', label: '数据分析', desc: '图表 · SQL · 洞察' },
            { href: '/digital-marketing', icon: '📣', label: '数字营销', desc: 'SEO · 社交媒体 · 广告活动' },
      ...
      <section className="text-center bg-indigo-600 rounded-2xl p-12 text-white">
        <h2 className="text-3xl font-bold mb-4">{post.cta?.heading ?? '发现你的准备度'}</h2>
        <p className="text-indigo-200 mb-8">{post.cta?.body ?? '面向14岁以上高中生的免费自适应评估。'}</p>
        <CtaLink href={post.cta?.href ?? '/apply'} label="zh_blog_cta" className="bg-white text-indigo-600 px-8 py-4 rounded-xl font-semibold hover:bg-indigo-50 transition-colors text-lg">
          {post.cta?.label ?? '开始免费评估'}
        </CtaLink>
      </section>
    </main>
  )

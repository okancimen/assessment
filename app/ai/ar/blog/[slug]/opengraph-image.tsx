...
import { getArabicPostBySlug } from '@/app/blog/posts-ar'
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = post?.shortTitle ?? 'أبحاث التدريب والمسار المهني'
  const tag = post?.tags[0] ?? 'تدريب'
